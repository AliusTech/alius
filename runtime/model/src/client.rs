...
impl LlmClient {
    ...
    pub fn new(settings: LlmSettings) -> Result<Self> {
        let provider: Box<dyn LlmProvider> = match settings.provider {
            ProviderType::Openai | ProviderType::BigModel | ProviderType::Custom => {
    ...
    pub fn list_models_blocking(&self, base_url: &str, api_key: &str) -> Vec<String> {
        ...
        let models = if self.provider_type == ProviderType::Anthropic {
    ...
    fn builtin_bigmodel_models(&self) -> Vec<String> {
        match self.provider_mode {
            Some(ProviderMode::Coding) => BIGMODEL_CODING_MODELS
                .iter()
                .map(|s| s.to_string())
                .collect(),
