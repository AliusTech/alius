...
const DEFAULT_MODEL_TOML: &str = include_str!("../config/defaults/model.toml");
...
/// Steps in the initialization wizard.
...
/// Persistent progress tracker for the init wizard.
...
impl InitProgress {
    /// Load progress from disk, or return a fresh tracker.
    pub fn load(alius_dir: &Path) -> anyhow::Result<Self> {
        let path = alius_dir.join("runtime").join("init_progress.json");
        if path.exists() {
    ...
    pub fn save(&self, alius_dir: &Path) -> anyhow::Result<()> {
        let dir = alius_dir.join("runtime");
        std::fs::create_dir_all(&dir)?;
        let path = dir.join("init_progress.json");
        std::fs::write(&path, serde_json::to_string_pretty(self)?)?;
        Ok(())
    }

    /// Mark a step as completed.
    pub fn mark_completed(&mut self, step: InitStep) {
    ...
    pub fn is_completed(&self, step: &InitStep) -> bool {
        self.completed_steps.contains(step)
    }

    /// Clear progress (reset).
    pub fn clear(alius_dir: &Path) -> anyhow::Result<()> {
        let path = alius_dir.join("runtime").join("init_progress.json");
        if path.exists() {
...
mod tests {
    ...
    fn test_reset_creates_all_config_files() {
        ...
        assert!(project_dir.join(".alius/config/model.toml").exists());
