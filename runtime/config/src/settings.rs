...
impl Settings {
    ...
    ///
    /// Writes both legacy `[llm]` section and new-system `[model]` section so
    /// both configuration systems can read from the same file.
    pub fn save_to_project_config(&self) -> anyhow::Result<()> {
        let config_path = ensure_project_config_dir()?;
        let legacy = toml::to_string_pretty(self)
            .map_err(|e| anyhow::anyhow!("Failed to serialize config: {}", e))?;

        let provider_str = serde_json::to_value(&self.llm.provider)
            .ok()
            .and_then(|v| v.as_str().map(|s| s.to_string()))
            .unwrap_or_default();

        let new_sections = format!(
            ...
            provider_str, self.llm.model
        );

        let content = format!("{legacy}{new_sections}");
        std::fs::write(&config_path, content)
...
pub struct LlmSettings {
    /// The LLM provider type (OpenAI, Anthropic, Google, BigModel, Xiaomi MiMo, or Custom).
    ...
    pub fn get_base_url(&self) -> String {
        ...
        match self.provider {
            ...
            ProviderType::BigModel => match self.provider_mode {
                Some(ProviderMode::Native) => "https://open.bigmodel.cn/api/anthropic".to_string(),
                _ => "https://open.bigmodel.cn/api/coding/paas/v4".to_string(),
            },
...
mod tests {
    ...
    fn test_base_url_deepseek() {
        ...
        assert_eq!(s.get_base_url(), "https://api.deepseek.com");

        let s = LlmSettings {
            provider: ProviderType::DeepSeek,
            provider_mode: Some(ProviderMode::Native),
            model: "test".to_string(),
            api_key: None,
            api_key_env: None,
            base_url: None,
        ...
        assert_eq!(s.get_base_url(), "https://api.deepseek.com/anthropic");
    }
