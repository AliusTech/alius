...
//! and response style.
//!
//! **Deprecated**: Built-in soul roles are superseded by formula-based souls
//! from `alius-souls`. The REPL falls back to these prompts only when no
//! formula soul is installed. Prefer `alius soul install <id>` or `/init`.

use protocol_interface::SoulRole;

/// Built-in soul roles available for selection.
///
/// **Deprecated**: Use `crate::formula::list_available_souls()` instead.
...
///
/// **Deprecated**: Prefer `crate::formula::load_soul_prompts()` for formula-based
/// souls. This function serves as a fallback when no formula soul is installed.
///
/// # Arguments
