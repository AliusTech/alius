...
function normalizeVersion(input) {
  ...
  const withoutRef = trimmed
    .replace(/^refs\/heads\/release\//, '')
    .replace(/^refs\/tags\//, '')
    .replace(/^release\//, '');
  return withoutRef.replace(/^v/, '');
}

function normalizeReleaseTag(input, version) {
  if (!input) return version;
  const trimmed = String(input).trim();
  const withoutRef = trimmed
    .replace(/^refs\/heads\/release\//, '')
    .replace(/^refs\/tags\//, '')
    .replace(/^release\//, '');
  return withoutRef || version;
}
...
const releaseTagInput = process.argv[3]
...
const RELEASE_TAG = normalizeReleaseTag(releaseTagInput, VERSION);
...
function generatePackageJson(platform) {
  return {
    ...
    binary: {
      ...
      downloadUrl: `https://github.com/AliusTech/alius/releases/download/${RELEASE_TAG}/${ARTIFACT_MAP[platform.name]}`,
    },
...
const RELEASE_TAG = pkg.binary.releaseTag || '${RELEASE_TAG}';
