...
pub fn workflows_dir() -> std::path::PathBuf {
    ...
    std::path::PathBuf::from(home).join(".alius").join("workflows")
}
...
pub async fn execute_step(step: &Step, ctx: &ExecutionContext) -> Result<StepResult> {
    match step.step_type {
        ...
        StepType::Tool => {
            ...
            Ok(StepResult {
                step_id: step.id.clone(),
                output: format!("[tool:{}] args={}", tool_name,
...
pub async fn execute_workflow(workflow: &Workflow) -> Result<ExecutionContext> {
    ...
    for step in &workflow.steps {
        ...
        println!("    {} {}", if result.success { "OK" } else { "ERR" },
            if result.output.len() > 100 { &result.output[..100] } else { &result.output });
