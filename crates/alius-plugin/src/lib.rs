    ...
    for entry in std::fs::read_dir(&dir)? {
        ...
        if manifest_path.exists() && wasm_path.exists() {
            ...
            if let Ok(manifest) = toml::from_str::<PluginManifest>(&content) {
                plugins.push(Plugin { manifest, wasm_path });
            }
        }
    }
...
pub fn call_plugin_tool(plugin: &Plugin, tool_name: &str, args: &serde_json::Value) -> Result<serde_json::Value> {
    ...
    let call_tool = instance.get_typed_func::<(i32, i32, i32, i32), i32>(&mut store, "alius_plugin_call_tool")
    ...
    // Write args
    memory.data_mut(&mut store)[args_ptr as usize..args_ptr as usize + args_bytes.len()].copy_from_slice(args_bytes);

    let result_ptr = call_tool.call(&mut store, (
        name_ptr as i32,
    ...
    let result_bytes = &memory.data(&store)[result_ptr as usize + 4..result_ptr as usize + 4 + result_len];
