        ...
        match action {
            ...
            "done" => {
                let id = args["id"].as_u64()
                    .ok_or_else(|| AliusError::Agent("Missing 'id' for done".to_string()))? as usize;
            ...
            "remove" => {
                let id = args["id"].as_u64()
                    .ok_or_else(|| AliusError::Agent("Missing 'id' for remove".to_string()))? as usize;

                let todos = store.entry(session_id).or_default();
                if let Some(pos) = todos.iter().position(|t| t.id == id) {
                    let removed = todos.remove(pos);
                    Ok(ToolResult::success(format!("Removed todo #{}: {}", id, removed.text)))
                } else {
                    Err(AliusError::Agent(format!("Todo #{} not found", id)))
                }
            }
