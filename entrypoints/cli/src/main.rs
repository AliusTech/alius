...
use alius_cli::{
    Cli, Command, ConfigCommand, CoreCommand, McpCommand, PluginCommand, SoulCommand,
    WorkflowCommand,
};
use anyhow::Result;
use clap::Parser;
...
pub async fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Command::Run { prompt, model }) => {
            ...
            runtime_tools::register_builtin_tools(&mut registry);

            let bridge = repl::protocol_bridge::ProtocolBridge::new(
                workspace_root,
                settings,
                client,
            ...
            bridge.send_message_streaming_with_mode(
                &prompt,
                protocol_interface::core::RuntimeMode::Chat,
                |delta| {
                    print!("{}", delta);
