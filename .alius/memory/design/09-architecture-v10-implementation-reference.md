...
- 明确 A2A、JSON-RPC、Cargo Features 的实现约束。
...
右侧的外部资源不是架构层，只是 Core Runtime 的外部依赖。底部的 Cargo Features 也不是运行时层，而是把同一套架构裁剪成 CLI、Desktop、IDE、A2A 等不同编译目标。
...
- JSON-RPC、Plugin RPC、A2A 都应只做协议适配，不复制 Agent Engine 逻辑。
...
| Shell Gate | shell/process/git 命令门禁 | command、args、cwd、workspace、policy | allow / deny / approval required | `AcceptEdits` 下检查命令、参数和作用范围，workspace 外读写需授权 |
...
| Security & Policy Manager | 审批、权限、sandbox、allowlist | tool request、policy、product capability | allow / deny / ask | `AcceptEdits` 下所有危险工具必须经过这里；`BypassPermissions` 是显式高风险例外 |
...
| 外部资源 | 调用方 | 用途 |
| --- | --- | --- |
| Model Provider APIs | Provider Manager | 远程模型推理 |
| Embedding APIs / Memory Gateway | Memory System | embedding、远程 memory 网关 |
...
```mermaid
flowchart LR
  POLICY["Feature Policy\ncli-full / desktop-planned / a2a"] --> BCLI["CLI 构建\ncargo build --release\nfeatures = cli-full/default"]
  POLICY --> BDESK["Desktop 规划构建\nfeatures = desktop-planned"]

  BCLI --> CLI["Alius CLI（含 TUI）\n完整 Core Runtime\nA2A 可运行时开关启用"]
  BDESK --> DESK["Desktop / JSON-RPC 规划接口"]
```
...
- `AcceptEdits` 下，`Tool Executor` 调用前必须经过 `Security & Policy Manager`。
- `AcceptEdits` 下，shell/process/git 调用必须经过 `Shell Gate`，检查命令、参数、cwd、路径、glob、symlink 和重定向。
- `AcceptEdits` 下，workspace 外读写必须授权；`BypassPermissions` 跳过 Alius 门禁但必须保留事件和审计可见性。
...
8. `AcceptEdits` 下 shell/process/git 必须经过 Shell Gate；`BypassPermissions` 必须作为显式策略进入 Tool Executor，不能被隐式触发。
9. Logging Manager 必须实时记录运行日志、错误、异常和审计。
10. A2A 是远端 Agent 协议，不是本地 plugin，也不是普通 MCP tool。
11. 外部资源只能通过对应 manager 访问，不能从产品层或接口层直接访问。
...
```mermaid
flowchart TB
  PRODUCT["任一产品入口\nCLI / IDE / Desktop / Third-party Agent"] --> INTERFACE["对应接口适配\nRust API / RPC / JSON-RPC / A2A"]
