...
[2026-06-15 17:45] Alius: runtime/tools/src/package.rs + runtime/tools/src/registry.rs - P3-2 Native tools enter default ToolRegistry: build_registry/build_registry_lossy now always register native tools first (shell, read_file, write_file, list_dir, edit_file); ToolRegistry::register rejects duplicate tool names so WASM plugins cannot shadow built-in tools; added unit tests for native registration and name-conflict rejection; added integration tests verifying default registry, confirmation preview for shell/write_file/edit_file in Plan vs Chat mode, workspace-internal execution, and external-path denial.
